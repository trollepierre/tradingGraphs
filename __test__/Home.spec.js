...
describe('>>>H O M E --- Snapshot',()=>{
    it('+++capturing Snapshot of Home', () => {
        const renderedValue = renderer.create(<Home tradingValues={tradingValues}/>).toJSON()
...
describe('>>>H O M E --- Shallow Render REACT COMPONENTS',()=>{
    ...
    beforeEach(()=>{
        wrapper = shallow(<Home tradingValues={tradingValues}/>)
    })
...
describe('>>>H O M E --- REACT-REDUX (Mount + wrapping in <Provider>)',()=>{
    ...
    it('+++ check Prop matches with initialState', () => {
       expect(wrapper.find(Home).prop('tradingValues')).toEqual(initialState.tradingValues)
    });
...
describe('>>>H O M E --- REACT-REDUX (actual Store + reducers) more of Integration Testing',()=>{
    ...
    it('+++ check Prop matches with initialState', () => {
        store.dispatch(updateTradingValues(tradingValues))
       expect(wrapper.find(Home).prop('tradingValues')).toBe(tradingValues)
    });
