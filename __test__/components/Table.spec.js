...
describe('>>>TABLE --- REACT-REDUX (Shallow + passing the {store} directly)', () => {
  const initialState = { updateTradingValues: { tradingValues: [{ stocks: 'somestocks' }] } };
  ...
  it('+++ check Prop matches with initialState', () => {
    expect(container.prop('tradingValues')).toEqual(initialState.updateTradingValues.tradingValues);
  });

});

describe('>>>TABLE --- REACT-REDUX (Mount + wrapping in <Provider>)', () => {
  const initialState = { updateTradingValues: { tradingValues }, updateSpecificValue: { specificValues: [] } };
  const mockStore = configureStore();
  let store;
  let wrapper;
  ...
  it('+++ check Prop matches with initialState', () => {
    expect(wrapper.find(Table).prop('tradingValues')).toEqual(initialState.updateTradingValues.tradingValues);
  });
});